document.addEventListener('DOMContentLoaded', function() {
    ...
    window.checkedState = new Map(); // Global map to store checked state
    ...
    function displayPage(page) {
            ...
            for (const item of imageArray) {
                ...
                if (count >= startIndex) {
                    for (const [name, path] of Object.entries(item)) {
                        ...
                        checkBox.classList.add("selectedImage");
                        checkBox.type = "checkbox";
                        checkBox.name = name;
                        checkBox.value = "test";
                        checkBox.checked = window.checkedState.get(`${cls},${name}`) || false; // Set checked state

                        checkBox.addEventListener('change', function() {
                            window.checkedState.set(`${cls},${name}`, checkBox.checked); // Update checked state
                        });
        ...
        checkedImages = document.querySelectorAll(".selectedImage");
        window.checkedImages = checkedImages; // Attach to window object
    ...
    clearButton.addEventListener('click', (event) => {
        event.preventDefault();
        window.checkedState.clear(); // Clear the map
        checkedImages.forEach(checked => {
